...
const LiteUpdate: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        const res = await axios.get<Student>(`http://localhost:3000/students/sqlite/${id}`);
        setStudent(res.data);
      } catch (err) {
        console.error(err);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`http://localhost:3000/students/sqlite/${student._id}`, student);
