...
const DBnosql: React.FC = () => {
  ...
  useEffect(() => {
    const getStudents = async () => {
      try {
        const res = await axios.get<Student[]>("http://localhost:3000/students/postgres");
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`http://localhost:3000/students/postgres/${id}`);
      ...
      <div className="bg-white shadow-lg rounded-xl p-8 w-full max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-semibold">
            Student Records <sub className="text-sm">(SQL)</sub>
          </h1>
          <button 
          onClick={()=>navigate("/sql/create")}
            ...
            {students.map((student) => (
              <tr key={student._id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3 flex justify-center gap-2">
                  <Pencil 
                  onClick={()=>navigate(`/sql/update/${student._id}`)}
