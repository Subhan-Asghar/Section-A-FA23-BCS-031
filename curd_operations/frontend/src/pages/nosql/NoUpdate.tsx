...
const NoUpdate: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        const res = await axios.get<Student>(`http://localhost:3000/students/mongo/${id}`);
        setStudent(res.data);
      } catch (err) {
        console.error(err);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`http://localhost:3000/students/mongo/${student._id}`, student);
      alert("Student updated successfully!");
      navigate("/nosql"); // redirect back to your table
    } catch (err) {
      console.error(err);
      alert("Update failed!");
    }
